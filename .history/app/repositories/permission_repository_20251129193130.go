...
// GetAllPermissions -> ambil semua permission
...
// GetPermissionByID -> ambil permission by id
...
// GetPermissionsByIDs -> ambil banyak permission (dipakai saat login convert ids->permission)
func GetPermissionsByIDs(ctx context.Context, ids []string) ([]models.Permission, error) {
	if len(ids) == 0 {
		return []models.Permission{}, nil
	}
	rows, err := database.DB.Query(ctx,
		`SELECT id, name, resource, action, description FROM permissions WHERE id = ANY($1)`, ids)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
// CreatePermission -> tambah permission (ADMIN)
func CreatePermission(ctx context.Context, p *models.Permission) error {
	_, err := database.DB.Exec(ctx,
		`INSERT INTO permissions (id, name, resource, action, description) VALUES ($1,$2,$3,$4,$5)`,
		p.ID, p.Name, p.Resource, p.Action, p.Description)
	return err
}

// UpdatePermission -> update permission (ADMIN)
func UpdatePermission(ctx context.Context, p *models.Permission) error {
	_, err := database.DB.Exec(ctx,
		`UPDATE permissions SET name=$2, resource=$3, action=$4, description=$5 WHERE id=$1`,
		p.ID, p.Name, p.Resource, p.Action, p.Description)
	return err
}

// DeletePermission -> hapus permission (ADMIN)
