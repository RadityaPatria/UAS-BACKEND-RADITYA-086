...
// CreateAchievementReference -> insert reference (status draft) setelah simpan ke Mongo
...
// GetAchievementReferenceByID -> ambil reference by id
func GetAchievementReferenceByID(ctx context.Context, id string) (*models.AchievementReference, error) {
	...
	err := row.Scan(&ar.ID, &ar.StudentID, &ar.MongoAchievementID, &ar.Status,
		&ar.SubmittedAt, &ar.VerifiedAt, &ar.VerifiedBy, &ar.RejectionNote, &ar.CreatedAt, &ar.UpdatedAt)
	if err != nil {
		return nil, err
	}
	return &ar, nil
...
func UpdateAchievementReferenceStatus(ctx context.Context, id string, status string) error {
	...
	case models.StatusSubmitted:
		query = `UPDATE achievement_references SET status=$2, submitted_at=NOW(), updated_at=NOW() WHERE id=$1`
	case models.StatusVerified:
		query = `UPDATE achievement_references SET status=$2, verified_at=NOW(), updated_at=NOW() WHERE id=$1`
	case models.StatusRejected:
		query = `UPDATE achievement_references SET status=$2, updated_at=NOW() WHERE id=$1`
	default:
		// unsupported status -> still try generic update
		query = `UPDATE achievement_references SET status=$2, updated_at=NOW() WHERE id=$1`
	}
...
// SetAchievementRejectionNote -> set catatan penolakan (dosen)
func SetAchievementRejectionNote(ctx context.Context, id string, note string) error {
	_, err := database.DB.Exec(ctx,
		`UPDATE achievement_references SET rejection_note=$2, status='rejected', updated_at=NOW() WHERE id=$1`,
		id, note)
	return err
}

// GetAchievementReferencesByStudentIDs -> ambil references untuk list student ids (admin/dosen)
func GetAchievementReferencesByStudentIDs(ctx context.Context, studentIDs []string) ([]models.AchievementReference, error) {
	rows, err := database.DB.Query(ctx,
		`SELECT id, student_id, mongo_achievement_id, status, submitted_at, verified_at, verified_by, rejection_note, created_at, updated_at
		 FROM achievement_references WHERE student_id = ANY($1)`, studentIDs)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&ar.ID, &ar.StudentID, &ar.MongoAchievementID, &ar.Status,
			&ar.SubmittedAt, &ar.VerifiedAt, &ar.VerifiedBy, &ar.RejectionNote, &ar.CreatedAt, &ar.UpdatedAt); err != nil {
			return nil, err
		}
		list = append(list, ar)
	}
