...
// ========================================================================
// UPDATE ACHIEVEMENT (Mahasiswa) — FR Bagian CRUD
// ========================================================================
func UpdateAchievement(c *fiber.Ctx) error {
    ...
    if err := repositories.UpdateAchievementMongo(ctx, mongoID, payload); err != nil {
        return c.Status(500).JSON(fiber.Map{"error": err.Error()})
    }

    // Update updated_at in PostgreSQL
    ref.UpdatedAt = time.Now()
    if err := repositories.UpdateAchievementReferenceTimestamp(ctx, refID); err != nil {
        return c.Status(500).JSON(fiber.Map{"error": err.Error()})
    }
