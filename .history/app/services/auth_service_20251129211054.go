...
func Login(ctx context.Context, identifier string, password string) (*models.AuthResponse, error) {
	...
	// 4. Ambil nama role
	role, err := repositories.GetRoleByID(ctx, user.RoleID)
	if err != nil {
		return nil, errors.New("role not found")
	}

	// 5. Ambil permission_id dari role_permissions
	permissionIDs, err := repositories.GetPermissionIDsByRoleID(ctx, user.RoleID)
	if err != nil {
		return nil, err
	}

	// 6. Convert permission_id → permission detail
